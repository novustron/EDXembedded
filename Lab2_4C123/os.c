...
int OS_AddPeriodicEventThreads(void(*thread1)(void), uint32_t period1,
  ...
  // For consistency
//	tcbs[4].next = &tcbs[3]; // "If you run Task0, then Task5 will now run. If you run Task1, then Task2 will run
//  tcbs[5].next = &tcbs[0]; // 
//	SetInitialStack(3); Stacks[4][STACKSIZE-2] = (int32_t)(thread1); // PC
//	SetInitialStack(4); Stacks[5][STACKSIZE-2] = (int32_t)(thread2); // PC						
	
   periodic_task0 = thread1;
   periodic_task1 = thread2;
...
void Scheduler(void){ // every time slice
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
	...
	if(TheTime0 == tcbs[4].period)
	{
		// Function Call
		(*periodic_task0)();
		TheTime0 = 0; 
	}
	if(TheTime1 == tcbs[5].period)
	{
		// Function Call
		(*periodic_task1)();
		TheTime1 = 0; 
	}
		// call short bounded function
	TheTime0++;
	TheTime1++;
  RunPt = RunPt->next;
